...
public class EntradadeProdutoDao {
    ...
    public void salvar (EntradadeProdutoM entradadeproduto) throws SQLException{
        ...
        String sql;
        sql = "insert into EntradadeProduto set Id = ?, idproduto = ?, Data = STR_TO_DATE( ?, \"%d/%m/%Y\" ), Hora = ?, Quantidade = ?, Detalhes = ?";
        ...
        pst.setString(6, entradadeproduto.getDetalhes());
        pst.execute();
        pst.close();
    }
    ...
    public void alterar(EntradadeProdutoM entradadeproduto) throws SQLException{
        ...
        pst.setString(5, entradadeproduto.getDetalhes());
        pst.setInt(6,entradadeproduto.getId());
        pst.execute();
        pst.close();
     }
    ...
    public List<EntradadeProdutoM> listaTodos() throws SQLException{
        ...
        sql = "select Id, IdProduto, Data = STR_TO_DATE( ?, \"%d/%m/%Y\" ) AS Data, Hora, Quantidade, Detalhes from EntradadeProduto";
        pst = Conexao.getInstance().prepareStatement(sql);
    ...
    public EntradadeProdutoM busca(int id) throws SQLException{
        ...
        sql = "select Id, IdProduto, Data = STR_TO_DATE( ?, \"%d/%m/%Y\" ) AS Data, Hora, Quantidade, Detalhes from EntradadeProduto where id = ?";
        pst = Conexao.getInstance().prepareStatement(sql);
        pst.setInt(1, id);
        ResultSet rs = pst.executeQuery();
        while(rs.next()){
    ...
    public EntradadeProdutoM buscaNome(String nome) throws SQLException{
        ...
        String sql;
        EntradadeProdutoM entradadeproduto = null;        
        sql = "select Id, IdProduto, Data = STR_TO_DATE( ?, \"%d/%m/%Y\" ) AS Data, Hora, Quantidade, Detalhes from EntradadeProduto where nome = ?";
        pst = Conexao.getInstance().prepareStatement(sql);
    ...
    public List<EntradadeProdutoM> buscaNomeLista(String Nome) throws SQLException{
        ...
        List<EntradadeProdutoM> ListaEntradaProduto = new ArrayList<>();
        String name = "%"+Nome+"%";
        sql = "select Id, IdProduto, Data = STR_TO_DATE( ?, \"%d/%m/%Y\" ) AS Data, Hora, Quantidade, Detalhes from EntradadeProduto where nome like ?";
        pst = Conexao.getInstance().prepareStatement(sql);
